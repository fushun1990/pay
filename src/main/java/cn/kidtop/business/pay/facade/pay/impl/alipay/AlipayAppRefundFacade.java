 ...
 */
@Component
public class AlipayAppRefundFacade extends Refund{
	...
	protected String refundRequest(RefundParamDTO refundParamDTO,RecordRefundDTO recordRefundDTO) {
		...
		refundRequest(map,refundParamDTO,recordRefundDTO);
	 ...
	 * @records <p>  fushun 2016年9月14日</p>
	 */
	private Map<String, String> getRequestData(RefundParamDTO refundParamDTO) {
		
		Map<String, String> map=new HashMap<>();
		map.put("out_trade_no", refundParamDTO.getRefundFrom().getPreStr()+refundParamDTO.getOutTradeNo());
		map.put("refund_amount", String.valueOf(refundParamDTO.getRefundMoney()));
		if( refundParamDTO.getRefundReason()!=null){
			map.put("refund_reason",refundParamDTO.getRefundReason());
		}
		map.put("out_request_no", refundParamDTO.getRefundNo());
	 ...
	 */
	private void refundRequest(Map<String, String> map,RefundParamDTO refundParamDTO,RecordRefundDTO recordRefundDTO){

		AlipayTradeRefundResponse response=payRecordRefundAliPay(map, refundParamDTO);
		recordRefundDTO.setPayNo(response.getTradeNo());
		
		validatorServiceResponse.validatorServiceResponse(response, errorMap);
	
	}
	
	private AlipayTradeRefundResponse payRecordRefundAliPay(Map<String, String> map,RefundParamDTO refundParamDTO){
